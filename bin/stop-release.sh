...
# shell options touched). Run directly with --selftest to exercise the pure core.
#
# EXPORTS:
#   release_verdict <held-seconds> <releases-so-far> <cool-off-seconds> <max-releases>
#                             → HOLD | RELEASE | ESCALATE
#
# Callers: bin/dev-loop.sh (the backlog park), bin/pr-poller.sh (the exhausted fitness review + the
# fixer's no-progress stop). The full enumeration of the loop's stopping conditions — and which release
# each one carries — is STOPS.md, audited by bin/stop-audit.sh.
# Covered by stop-release.test.sh. Control-plane (the loop's stop/release policy). MUST be tracked 100755.
