...
#
# EVERY `all`/`host` RUN FILES A TICKET, and that cost is disclosed rather than hidden: there is no
# dedup here, so the CADENCE is what bounds it. The poller's immutability tick (#317) is the only
# automatic caller and fires roughly daily (IMMUTABILITY_PROBE_EVERY, 2880 sweeps), so the steady-state
# cost is ~1 control-repo issue per day — until feat-04 deploys, each one a ticket the host rejects.
# A human running it by hand pays one ticket per run.
#
...
# (`OBJECTIVE="#310"` lived here to be printed by the STAGED host stub this change replaces. Its only
# reader is gone, so it goes with it rather than lingering as a name nothing reads.)
...
#
# THE NEWEST HEADER WINS (`tail -1`), because host_comments feeds this the whole thread newest LAST. A
# ticket normally carries exactly one verdict, so this is invisible today — but reading `head -1` off a
# thread documented as newest-last would let a re-run's SUPERSEDED first verdict outrank the current one,
# and a probe whose whole claim is "this is what the host measured" must never report the stale answer.
host_verdict_of(){
  local v
  v="$(printf '%s\n' "${1:-}" \
...
# are accepted so the host may report in whatever unit reads well. `tail -1` for the same reason as
# above: the age must belong to the verdict that won, not to a superseded one earlier in the thread.
host_cached_age(){
  local raw n unit
  raw="$(printf '%s\n' "${1:-}" \
         | sed -n 's/.*[Cc]ached[[:space:]]*(\([0-9]\{1,\}[smhd]\{0,1\}\)).*/\1/p' | tail -1)"
...
# host_comments <issue-number> → every comment body on the ticket, newest LAST (the order the GitHub API
# returns and the order host_verdict_of's `tail -1` is written against). Read-only.
host_comments(){
...
selftest(){
  ...
  # host_comments feeds the thread NEWEST LAST, so the newest header must win: a superseded first
  # verdict outranking the current one would report the stale answer as this run's measurement.
  ck "on a multi-verdict thread the NEWEST wins" \
     "$(host_verdict_of 'immutability-probe: GREEN'$'\n''immutability-probe: RED — a re-run found residue')" "RED"
  ...
  ck "the age belongs to the NEWEST verdict" \
     "$(host_cached_age 'cached (90000)'$'\n''immutability-probe: GREEN cached (60)')" "60"
