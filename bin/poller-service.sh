...
#
# Env: POLLER_ARMED (default 0), POLLER_REPO, POLL_INTERVAL, FITNESS_SAME_IDENTITY, FITNESS_LOGIN — all
# pass through to pr-poller.sh (entrypoint.sh forwards them across the distrobox boundary; `distrobox
# enter` does not inherit the base env). This wrapper only adds readiness-waiting + restart-on-death.
set -uo pipefail
