...
**Why iterating costs almost nothing — the caches are kept, the candidate is thrown away.** Throwing away a test build deletes **only** that one candidate image and its temp tree — it never deletes the caches, and the caches aren't tied to a particular PR or commit; they're shared across every iteration. Two caches sit on the box's own writable storage and do the work. First, a **layer cache**: if a change only touches the late, light layers (a script, a config tweak), the heavy step that installs packages doesn't run at all, so nothing is downloaded — and that cache survives the candidate being deleted. Second, a **package cache** mounted into the build: even when a change *does* alter what gets installed (say, adding a tool), the install step re-runs but the packages come straight from that cache instead of being fetched again — measured about three times faster (94s down to 33s); only a genuinely new package is downloaded, once, and then it too is cached. Each test build is also kept separate from every other — its own temp tree, its own uniquely-named candidate image and run container — so parallel or back-to-back builds never step on each other, and because the caches are keyed by content they can never hand back the wrong version. And because the VPS has limited disk, three safety nets keep storage in check: the candidate cleans itself up whether the test passes, fails, or crashes; a sweeper reaps anything a hard crash leaves behind; and a size/age cap keeps the two caches from ever growing without bound.
