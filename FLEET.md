...
(no required reviews or status checks) is active on both repos — it forces every change
...
daily**) compares this payload across both public repos and **fails the build on any drift** — so
it cannot silently diverge. It once did: the self-update lockout landed in `fedora-dev` but was missing
from the other box until an audit caught it; the parity check is what makes that recurrence
impossible.
...
The axes are independent. **`fedora-dev` and `fedora-bootstrap` differ on BOTH axes** — `fedora-dev`
is a container and the sole merger; `fedora-bootstrap` is the host and a proposer/**DENY** live-gate
client. The "container + host" split is Axis A; the "1 merger + 1 proposer" split is Axis B; and the
...
1. **Intake & route.** A request lands on the box that *owns* the affected repo — the box that can
   ...
   and can live-diagnose. A repo a box neither owns nor can
...
2. **Develop → open PR.** The owning box develops on a branch and opens a PR against `main`. The PR
   **is** the work item and the handoff token (see *The ticket system*). **Feature-branch pushes are
   ...
   merge verbs). `fedora-bootstrap` **stops here** — it is PR-only.
3. **CI build-only check.** On the image repo (`fedora-dev`), `build.yml` fires on
...
can't live-diagnose, or a task parks mid-flight), open a **GitHub
...
- **STOP at the PR** — `fedora-bootstrap` is PR-only; its refspec-aware
  ...
  merge verb (this box never merges — there is nothing to approve, so the gate denies rather than
...
- **Claude-code guard payload** (the `managed-settings.json` deny-list + self-update lockout, the
  ...
  in both, CI-enforced** by `bin/fleet-guard-parity.sh` (push/PR + daily). This is the *invariant*
...
- **Sources** (dnf → vendor `.repo` → AppImage/`.war`, GPG/sha-verified) · **no secrets in image
  layers** · **headless everywhere** (software-GL); sensitive ports tailnet-only.
