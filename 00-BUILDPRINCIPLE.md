...
## BP3 — CAPABILITY-RELATIVE MINIMALISM
...
## BP4 — IMMUTABLE-HOST / CONTAINERISE-EVERYTHING
...
## BP8 — TEST-QUALITY / MUTATION-PROVEN
...
## BP9 — DOCUMENTATION-DRY
**One authoritative home per concept; every other mention is a one-line pointer or deleted.** Evidence and benchmarks live only in the principle they prove; fleet-wide-identical blocks are enforced identical (the `fleet-guard-parity.sh` precedent). This binds every spec/doc the apparatus authors, this Trinity included. **ENFORCEMENT — REVIEW, not machine:** the principle is binding and is enforced by the independent fitness review, which treats a doc asserting behaviour the code does not have as **(c) UNTRUE**. Judging whether two differently-worded passages state one concept is review work, not a text match, so there is no machine near-duplicate detector. Cross-repo fleet-identical blocks are machine-enforced by `bin/fleet-guard-parity.sh`.
