...
## BP9 — DOCUMENTATION-DRY (new — gap 5)
**One authoritative home per concept; every other mention is a one-line pointer or deleted.** Evidence and benchmarks live only in the principle they prove; fleet-wide-identical blocks are enforced identical (the `fleet-guard-parity.sh` precedent). This binds every spec/doc the apparatus authors, this Trinity included. **ENFORCEMENT — REVIEW, not machine (stated 2026-07-14; tooling landed 2026-07-19 #206, DELETED 2026-07-29 #274 STEP 5):** the principle is binding and is enforced by the independent fitness review, which treats a doc asserting behaviour the code does not have as **(c) UNTRUE**. The machine drift-audit that briefly backed it (`bin/doc-dry-audit.sh`) flagged only clauses duplicated **verbatim** — by its own admission silent once two copies had been reworded, i.e. blind to the drift it existed to catch — and no merge check ever required it, so it gated nothing. Fuzzy near-duplicate detection is therefore **considered and dropped, not a pending follow-up**: judging whether two differently-worded passages state one concept is review work, not a text match. Cross-repo fleet-identical blocks remain machine-enforced by `bin/fleet-guard-parity.sh`.
