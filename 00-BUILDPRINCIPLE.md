...
## BP9 — DOCUMENTATION-DRY (new — gap 5; principle + drift-audit tooling `bin/doc-dry-audit.sh`)
**One authoritative home per concept; every other mention is a one-line pointer or deleted.** Evidence and benchmarks live only in the principle they prove; fleet-wide-identical blocks are enforced identical (the `fleet-guard-parity.sh` precedent). This binds every spec/doc the apparatus authors, this Trinity included. **STATEMENT scope (2026-07-14; tooling landed 2026-07-19, #206):** the principle is binding now; the **automated cross-document drift-audit** — `bin/doc-dry-audit.sh`, the intra-repo companion to `fleet-guard-parity.sh` — now mechanically flags a concept stated verbatim in ≥2 docs (accepted-overlap baseline in `doc-dry-allow.txt`). It catches the duplication that *precedes* drift; a concept whose two homes have already been reworded into a near-duplicate stays review-enforced (fuzzy near-duplicate detection is a documented follow-up).
