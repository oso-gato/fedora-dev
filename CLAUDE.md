...
BEFORE it reaches Arthur, label the PR `live-validate`; the host builds it disposably and runs Gate B,
then comments a **GREEN/RED verdict** — it **comments, NEVER merges**. Full mechanics (org-wide
discovery, the structural guard, the PARSED-never-executed `.live-gate` contract, loopback fences,
per-SHA dedup, iterate-on-RED/SUPERSEDE/build-upon-GREEN, dynamic enrolment): **THE FLEET**
(`policy/CLAUDE.md`) + step 2 of TICKETS & THE DEV LOOP below.
...
5. **Merge on APPROVE.** On Arthur's click I merge to `main` — **any PR, any author including my own, control-plane PRs included**; control-plane PRs merge on the **same single click**. Arthur may also merge on GitHub himself. The gate mechanics — the refspec-aware in-session `gate-push.sh` (feature pushes autonomous; any main-touching push plus the merge verbs → clickable `ask`, no approval-marker, I cannot self-approve), the `require-PR` floor, `main` not branch-protected with no CI label-gate, and control-plane changes kept STANDALONE + FLAGGED in the merge TLDR — are gospel in `policy/CLAUDE.md` (THE FLEET; the gate is the SOLE control plane).
