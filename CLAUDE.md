...
| bin/dev-plan.sh | the AUTONOMOUS PLANNER (apparatus spec #135 R2) — the front of the dev loop. `dev-plan.sh <repo> <spec-issue#>` turns a HUMAN-CONFIRMED objective/spec issue into `backlog`-labelled FEATURE issues, one per feature. R1 discipline: refuses a spec that lacks the `approved` label / a maintainer `CONFIRMED` comment (fail-closed — the confirmed set is what fitness grades against). Determinism split (design law): the JUDGMENT ("what features") is ONE bounded `claude -p` that WRITES each feature to a file and creates NO issues; the ACTION (filing the `backlog` issues) is the plain-shell harness reading those files → auditable, model touches no GitHub state. BLOCKED/no-progress surface a dev-task question on the spec issue (never an approval); a plan-summary comment links the filed issues (R5). Idempotent via a per-(repo,spec) `.planned` marker. `PLAN_CLAUDE` overridable for the mock test; `--selftest` covers the pure helpers |
| bin/dev-loop.sh | the DEV-LOOP DRIVER (apparatus spec #135 work-plan P3) — closes the human out of the per-feature loop. `dev-loop.sh <repo>` (one pass) / `--watch <repo>` (timer, `LOOP_INTERVAL`) enumerates the OPEN `backlog`-labelled issues the planner produced and runs `bin/dev-author.sh` over each; the author + host-live-gate → fitness → poller pipeline do the build/validate/merge, so the driver writes NO such logic — it only SEQUENCES the author across the backlog (it is to dev-author what poller-service is to pr-poller). Deterministic + idempotent: dev-author's own marker/PR guards make a re-run a no-op for anything authored, so it is timer-safe; a BLOCKED/non-zero author is logged and the loop CONTINUES (one stuck feature never wedges the rest); `MAX_PER_PASS` caps a pass (defers, never drops). `DEV_AUTHOR` overridable for the mock test; `--selftest` covers the pure helpers |
| dev-loop.test.sh + dev-plan.test.sh | MOCK dry-runs of the driver + planner (stub gh + author/claude on PATH; assert backlog-drain/continue-past-stuck for the loop, and confirmed-guard/file→issue/BLOCKED/unconfirmed paths for the planner). No GitHub/network/model |
