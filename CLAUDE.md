...
| 8 | CI + LAYERED CADENCE | `.github/workflows/build.yml` publishes the base image to GHCR + **key-signs** it (the Principle 11 signature flow): on push, on the 15th monthly (`--no-cache`), and on manual dispatch. Built-in token only. The IN-CONTAINER claudebox refreshes daily on its own timer + ad-hoc triggers; it never touches CI. |
...
| 11 | SIGNATURE FLOW (fleet baseline — every image repo builds this in) | Every image this repo publishes is **key-signed in CI by digest** with the fleet cosign **private key** (CI secrets `COSIGN_PRIVATE_KEY` + `COSIGN_PASSWORD`), and is **host-enforceable** via podman-native `policy.json → sigstoreSigned { keyData: <fleet public key> }` (**no cosign on the host**). **Key-based, NEVER keyless** — podman/`containers-image` `policy.json` matches only a `subjectEmail`, never a GitHub-Actions workflow-URI identity, so keyless can never be enforced host-side. This is a **ground-rule baseline built into every fleet image repo from the start** (peer of Principle 7's `run.sh` contract), and the cryptographic ground the autonomous pre-merge dev loop stands on: `fedora-bootstrap` runs *un-merged* candidate images and must verify CI provenance before running them. To implement in a repo: `build.yml` signs with `cosign sign --key env://COSIGN_PRIVATE_KEY`; the host pins the matching public key in `policy.json`. |
