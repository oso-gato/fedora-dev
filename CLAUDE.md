...
| bin/pr-poller.sh | the DEV-SIDE POLLER (Step 5): plain-shell watcher (NO model in the loop) that routes each open PR by (host verdict, tier, fitness verdict) via the pure `plan()` core — RED → bounded headless fixer on the FEATURE branch; GREEN+unreviewed → `fitness-review.sh`; GREEN+PASS → `auto-merge.sh` for ANY tier (**dry-run unless `POLLER_ARMED=1`**); **ZERO-GATE (2026-07-10): tier no longer routes to a human — `plan()` merges every host-GREEN + fitness-PASS PR regardless of tier**; only a fitness ESCALATE (reviewer defers) or an unknown fitness token still surfaces. **THE FIXER IS ISOLATED AND THE HARNESS OWNS ITS GIT (#148, mirroring `dev-author.sh`):** `run_fixer` cuts a THROWAWAY worktree via `fresh-tree.sh` pinned to the PR's OWN head (`FD_BASE_REF=origin/<ref>`) and runs the model THERE — **never** in the shared live clone the poller itself executes from (a `git checkout -b` there moves the running poller's HEAD off main and can leak a commit onto a parallel actor's branch — the 2026-06-28 incident; `policy/CLAUDE.md` makes the dedicated worktree mandatory). Isolation is **guaranteed, not the model's discretion**, and **fail-closed: there is NO shared-clone fallback** — a fixer that cannot be isolated is surfaced and never runs. The model **commits and does not push**; the SHELL pushes to an explicit feature refspec (which by construction cannot name main — a main/master head ref is refused outright) and then **VERIFIES THE LANDING against `origin`** (a push's exit code is not proof — `ls-remote` must read the ref back as our commit). Every outcome is DISTINGUISHABLE in the log — `FIX LANDED` / `NO-COMMIT` / `BLOCKED` / `TREE-FAILED` / `PUSH-FAILED` / `NOT-LANDED` / `SKIP-HEAD-MOVED` — and every non-landing one pushes nothing and surfaces honestly (the retired `(if pushed)` shrug made a silent no-op indistinguishable from a landed fix, so the next sweep's no-progress stop parked the PR on a FALSE `blocked`). A head that moved past the gated sha is SKIPPED (it re-gates on its own), and the worktree is reaped on every path incl. the `--watch` signal trap (Principle 10). Progress-based fixer stop (no silent quit), flock singleton, `--selftest` for the pure core, `pr-poller.test.sh` for the fixer's real sweep. Every host/fitness verdict is bound to the PR's head SHA and the merge is `--match-head-commit`-pinned (a fresh, ungated head never inherits a prior verdict). Each sweep FIRST retires SUPERSEDED PRs deterministically: a MERGED PR whose body carries a WHOLE LINE that is exactly `Supersedes #N[, #M…]` (same-repo; markdown-aware all-or-nothing parse — backticked/blockquoted/mid-sentence/malformed lines and fenced/code-indented blocks never act) closes a still-OPEN #N with a comment — the sanctioned close path (the interactive agent is classifier-denied `gh pr close` on PRs it didn't create, run-003 lesson b). Runs even DISARMED: the authorizing event is the superseder's human-gated merge; a close is reversible and never touches main (`RETIRE_LOOKBACK` most-recently-updated merged PRs, default 15, so a late-merging parked PR still enters the window; scan-once + retired state markers; a human REOPEN is durable — a prior retire comment on the PR blocks any re-close even after state loss; fail-closed to no-op on issue numbers / cross-repo refs / non-open targets / transient API failures, close failures logged as RETIRE FAILED). Sweep is fetch-BATCHED for the 10s cadence: ONE TSV list call carries number+ref+sha, ONE sha-bound comments call per open PR; a PARKED GREEN PR (acted marker present) is terminal-state-skipped at the same 1 call/sweep; tier(files) + fitness comments are fetched ONLY while a GREEN PR's routing is still pending, rc-checked so a transient fetch failure SKIPS that PR for that sweep (retry next) instead of misrouting (no tier=A-by-failure PRESENT stickiness, no spurious fitness re-runs); the tier feed is newline-terminated with a zero-file guard (fail-closed to A) and `tier-classify.sh --stdin` keeps a final unterminated line; steady state ≈ 360×(2+N) calls/h, ceiling ~10 sustained open PRs. |
| pr-poller.test.sh | MOCK end-to-end drive of the REAL sweep in `bin/pr-poller.sh` (`--once`), covering the RED-FIXER's isolation + landing contract (#148) — the surface `--selftest` never reached (it exercises only the pure core, which is how both defects shipped). Stubs gh + the fixer + `git push` on PATH (à la `dev-author.test.sh`) but drives REAL git — a real bare `origin`, a real clone, a real `fresh-tree.sh` worktree — so the push and the origin read-back are honest. Asserts each outcome: LANDED (model ran in an ISOLATED worktree cut from the PR's own head, harness pushed, origin actually advanced, **shared clone never moved**) / BLOCKED / NO-COMMIT / TREE-FAILED (no clone AND fresh-tree failure — fail-closed, fixer never spawned) / PUSH-FAILED / NOT-LANDED (push exits 0 but origin never moved) / SKIP-HEAD-MOVED, plus worktree reaping on every path. **MUTATION-CHECKED:** it restores each defect in a COPY of the poller (shared-clone `cwd`; landing verification dropped) and re-runs itself against it, asserting it FAILS — a test that passes against the pre-fix code is worth nothing. `bash pr-poller.test.sh` → exit 0 = all pass. No GitHub/network/model. Run after touching the fixer |
