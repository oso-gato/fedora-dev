...
3. Maintainer approves (clickable); the agent then merges (the canonical merge rule — see policy/CLAUDE.md DO NOT).
...
| 10 | PROPOSE-AND-COMMIT | The in-box agent grows `distrobox.ini`/`policy/`/`box-rebuild.sh`/etc. only by editing the LIVE clone at `/home/core/.local/share/fedora-dev/` and opening a PR. On the maintainer's clickable approval the agent merges (the canonical merge rule — policy/CLAUDE.md DO NOT); the next box rebuild applies. Ad-hoc installs vanish on rebuild. |
