...
# $1 (no reliance on runuser env-preservation). All are colon-dash-defaulted downstream, so an unset
# OR EMPTY value safely falls back (POLLER_ARMED→0 = DISARMED; FITNESS_LOGIN→the fleet default;
...
# ---- G7: POLLER SELF-ARM + ARMING DURABILITY across a rebuild-to-spec --------------------------------
...
# merge loop (observed live 2026-07-21: the poller went down at the rebuild and never came back). TWO fixes,
# MIRRORING the DEV_LOOP self-arm rationale (a host-set flag a self-refresh cannot reproduce would need a
# MANUAL host arm, breaking self-arming):
#   (1) POLLER_ENABLED DEFAULTS ON below (:-1) — the poller self-arms (sweeps/reviews/routes) with the
#       apparatus, surviving any recreation. This is the SAFE half: it NEVER merges until POLLER_ARMED.
#   (2) POLLER_ARMED + the fitness knobs (the MERGE-arming half — kept EXPLICIT, never a blanket default) are
#       PERSISTED on the HOME VOLUME (survives recreation, like the live spec) whenever a deploy set them, and
#       RESTORED when the env is absent — so the operator's ONE deliberate arming survives every later rebuild.
...
if [ "${POLLER_ENABLED:-1}" = 1 ]; then
    echo "[poller] POLLER_ENABLED (default-on self-arm) — starting the dev-side poller in-box (armed=${POLLER_ARMED:-0})"
